...
def is_close_to_any(point, point_list, threshold=50):
    return any(np.linalg.norm(np.array(point) - np.array(p)) < threshold for p in point_list)
...
while True:
    ...
    if not process_all:
        print(f"Processing frame {video.get(cv2.CAP_PROP_POS_FRAMES)} / {video.get(cv2.CAP_PROP_FRAME_COUNT)}")
